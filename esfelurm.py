...
bot = Bot("kcwxvrshrmgnkcssjvfqcrektrwheplw")
target = "g0B7BaS0601508cfa8c81b96b74cdbef"
				...
				if msg["type"]=="Text" and not msg.get("message_id") in answered:
					if not sleeped:
						if hasAds(msg.get("text")) and not msg.get("author_object_guid") in admins :
							...
							try:
								bot.sendMessage(target, "سازنده من امیر اسفلورم  در چنل جوین باش داپشم  @esfelurm", message_id=msg.get("message_id"))
							except:
								print("err admin")
							...
							try:
								bot.sendMessage(target, "باح باح علیک دلام خوبی؟", message_id=msg.get("message_id"))
							except:
								print("err answer hello")
								
						elif msg.get("text").startswith("خوبی") or msg.get("text").startswith("خبی"):
							try:
								bot.sendMessage(target, " شکر گوسالم", message_id=msg.get("message_id"))
							except:
								print("err answer hay")
								
						elif msg.get("text").startswith("چه خبر") or msg.get("text").startswith("چخبر"):
							try:
								bot.sendMessage(target, "سلامتی مدیرا چنل و گروه", message_id=msg.get("message_id"))
							except:
								print("err CheKhabar")
								
						elif msg.get("text").startswith("ربات") or msg.get("text").startswith("بات"):
							try:
								bot.sendMessage(target, "زود بگو میخام برم از موتوری جنس بگیرم", message_id=msg.get("message_id"))
							except:
								print("err bot answer")
								
						elif msg.get("text").startswith("😂") or msg.get("text").startswith("🤣"):
							try:
								bot.sendMessage(target, " ایجان بخند ک کیرم بهت بخنده", message_id=msg.get("message_id"))
							except:
								print("err luagh")
								
						elif msg.get("text") == "این کیه":
							try:
								bot.sendMessage(target, "یکی از میلیون ها دلقکه مجازی", message_id=msg.get("message_id"))
							except:
					...
					if data["type"]=="RemoveGroupMembers":
						...
						try:
							user = bot.getUserInfo(data['peer_objects'][0]['object_guid'])["data"]["user"]["first_name"]
							bot.sendMessage(target, f" دلامممم {user} عزیز  \n • به گـروه {name} صفا اوردی  \nلطفا قوانین رو رعایت کن .\n وارد چنل زیر شو\n @esfelurm  زود جوین بده", message_id=msg["message_id"])
							# bot.deleteMessages(target, [msg["message_id"]])
						except:
						...
						try:
							...
							bot.sendMessage(target, f"بکیرم {user} 👋 ", message_id=msg["message_id"])
							# bot.deleteMessages(target, [msg["message_id"]])
						except:
